...
// Fonction qui calcule le poids d'un etat de jeu en se basant sur les compteurs
    ...
    if (max_Joueur == true)
    {
            ...
            for(n=0;n<longueur;n=n+2)
        {
         ...
         //   Capture_Pions((tab_fils[n]).Y+1,(tab_fils[n+1]).Y+1,(tab_fils[n]).X+1,(tab_fils[n+1]).X+1,&C_Ordi,copieTableau,'B');
            ...
            // evaluer l'etat
            Poids = minimax(copieTableau,prof_arbre -1 ,false, C_Ordi, C_Joueur);
     ...
     else
     {
         ...
         for(n=0 ; n<longueur ; n=n+2)
        {
            ...
            capture(copieTableau,(tab_fils[n+1]).X+1,(tab_fils[n+1]).Y+1,&C_Joueur);


      //      Capture_Pions((tab_fils[n]).Y+1,(tab_fils[n+1]).Y+1,(tab_fils[n]).X+1,(tab_fils[n+1]).X+1,&C_Joueur,copieTableau,'N');

            // evaluer l'etat
            Poids = minimax(copieTableau,prof_arbre -1, true, C_Ordi,C_Joueur );
...
void pionChoisi(char table[7][5] , pion tab_fils[], int cOrdinateur, int cJoueur,int L, pion *meilleurPion, pion *meilleurEmp, int reponse)
{
    ...
    for (n=0 ;n<L ; n=n+2) // pour chaque etat
    {
     ...
     //   Capture_Pions((tab_fils[n]).Y+1,(tab_fils[n+1]).Y+1,(tab_fils[n]).X+1,(tab_fils[n+1]).X+1,&compteurOr,copieTableau,'B');
        ...
        if(reponse == 1)
        {
            poids = minimax(copieTableau,4,false,compteurOr,compteurJ);
        }
        else
        {
            poids = alpha_beta(copieTableau,5,false,compteurOr,compteurJ, -INT_MAX, INT_MAX);
        }
